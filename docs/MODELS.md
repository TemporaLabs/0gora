...
2. The lineup favors **lab diversity** and a strong, on-theme **default**.
...
| 3 | `deepseek/deepseek-chat-v3-0324` | DeepSeek | grounded instruction-following | [pc.0g.ai/models/deepseek-v3](https://pc.0g.ai/models/deepseek-v3) |
...
DeepSeek-V3 (the `0324` chat checkpoint). **Why:** strong at grounded, instruction-following answers
(answer-only-from-context + clean citations — exactly 0Gora's RAG discipline), and a recognizable non-GLM
option.
...
- `zai-org/GLM-5-FP8` — previously the only model; retired from the picker in favor of `GLM-5.1-FP8`
  (still funded in the background as a fallback).
