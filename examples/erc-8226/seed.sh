...
# The instance id (from the config) tells the backend which agora's collection to
# ingest into — required when one deployment hosts several agoras. Empty = default.
instance="$(jq -r '.id // ""' "$CONFIG")"
count="$(jq '.corpus.seeds | length' "$CONFIG")"
echo "Seeding $count source(s) into $API (instance: ${instance:-default}) …"
...
while read -r seed; do
  ...
  payload="$(jq -nc --arg url "$url" --arg mode "$mode" --argjson max_pages "$max_pages" --arg instance "$instance" \
    '{url: $url, mode: $mode, max_pages: $max_pages, instance: $instance}')"
